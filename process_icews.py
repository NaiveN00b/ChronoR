...
def prepare_dataset(path, name):
    ...
    # write ent to id / rel to id，将字典{entity(rel)(ts):id}存储于data/dataset_name/ent_id(rel_id)(ts_id)
    ...
    for f in files:
        ...
        for lhs, rel, rhs, ts in examples:
            to_skip['lhs'][(rhs, rel + n_relations, ts)].add(lhs)  # reciprocals (t,r^-1,h,t)
